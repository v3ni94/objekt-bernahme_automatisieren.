"""objekt_anschrift_kandidaten (24.09.2026): Anschrift eines Objekts ohne Strasse aus den Seitentexten seiner
Dokumente vorschlagen; --echt setzt nur eindeutige Vorschlaege und nie ueber eine vorhandene Strasse."""

from __future__ import annotations

import hashlib
from io import StringIO

import pytest
from django.core.management import call_command
from django.utils import timezone

from apps.audit.models import AuditEvent
from apps.documents.models import Document, DocumentPage
from apps.objects.management.commands.objekt_anschrift_kandidaten import extract_addresses
from apps.objects.models import ManagedObject

pytestmark = pytest.mark.django_db
_n = {"i": 0}
BLOCK = (
    "Wohnungseigentümergemeinschaft\nAm Beispielpark 5\n12345 Beispielstadt\n\nSehr geehrte Damen und Herren,"
)
INLINE = "Betreff: Objekt Am Beispielpark 5, 12345 Beispielstadt, Hausgeld 2026"
FREMD = "Verwalterweg 1\n99999 Verwaltstadt\nTelefon [NAME]"
ANDERE = "Musterweg 3a\n54321 Musterdorf"


def _obj(nr, **kw):
    return ManagedObject.objects.create(
        object_number=nr, name=f"Objekt {nr}", management_type="weg", status="takeover", is_test=True, **kw
    )


def _doc(obj, *pages):
    _n["i"] += 1
    doc = Document.objects.create(
        object=obj,
        sha256=hashlib.sha256(f"{obj.pk}-{_n['i']}".encode()).hexdigest(),
        size_bytes=100,
        mime_type="application/pdf",
        original_name=f"d{_n['i']}.pdf",
        current_name=f"d{_n['i']}.pdf",
        source="upload",
        status="review",
        first_seen_at=timezone.now(),
    )
    for i, text in enumerate(pages, start=1):
        DocumentPage.objects.create(
            document=doc,
            page_no=i,
            text_source="text_layer",
            is_scan=False,
            text_content=text,
            char_count=len(text),
        )
    return doc


def test_extraktion_block_und_inline_ohne_anreden():
    found = extract_addresses(
        "\n".join([BLOCK, INLINE, FREMD, "Herr Mustermann", ANDERE, "Seite 1\n12345 Ort"])
    )
    keys = {(s.lower(), n, p) for s, n, p, _c in found}
    assert ("am beispielpark", "5", "12345") in keys
    assert ("verwalterweg", "1", "99999") in keys and ("musterweg", "3a", "54321") in keys
    assert not [f for f in found if "Herr" in f[0] or "Seite" in f[0]]
    assert (
        len([f for f in found if f[0].lower() == "am beispielpark"]) == 1
    )  # Block und Inline zaehlen einmal


def test_vorschau_und_setzen_bei_eindeutigem_vorschlag():
    obj = _obj("700")
    for _ in range(3):
        _doc(obj, BLOCK, "Seite 2 ohne Anschrift")
    _doc(obj, FREMD)
    _doc(obj, "Kein Text mit Anschrift")
    out = StringIO()
    call_command("objekt_anschrift_kandidaten", objekt="700", ausser="Verwalterweg", min_hits=3, stdout=out)
    text = out.getvalue()
    assert "Objekt 700 (weg): 5 Dokumente" in text
    assert "1. Am Beispielpark 5, 12345 Beispielstadt: 3 Dokumente (60 %)  <- eindeutig" in text
    assert "Verwalterweg" not in text and "wuerde Hauptanschrift setzen" in text
    obj.refresh_from_db()
    assert not obj.street

    out = StringIO()
    call_command(
        "objekt_anschrift_kandidaten", objekt="700", ausser="Verwalterweg", min_hits=3, echt=True, stdout=out
    )
    obj.refresh_from_db()
    assert (obj.street, obj.house_number, obj.postal_code, obj.city) == (
        "Am Beispielpark",
        "5",
        "12345",
        "Beispielstadt",
    )
    assert "1 gesetzt" in out.getvalue()
    assert (
        AuditEvent.objects.filter(
            action="object.update", entity_id=obj.pk, reason__contains="objekt_anschrift_kandidaten"
        ).count()
        == 1
    )
    # zweiter Lauf: erfasst, nichts ueberschrieben, kein zweiter Audit-Eintrag
    out = StringIO()
    call_command(
        "objekt_anschrift_kandidaten", objekt="700", ausser="Verwalterweg", min_hits=3, echt=True, stdout=out
    )
    assert "bereits erfasst" in out.getvalue() and "0 gesetzt" in out.getvalue()
    assert AuditEvent.objects.filter(action="object.update", entity_id=obj.pk).count() == 1


def test_nicht_eindeutig_wird_nicht_gesetzt():
    obj = _obj("701")
    for _ in range(2):
        _doc(obj, BLOCK)
    for _ in range(2):
        _doc(obj, ANDERE)
    out = StringIO()
    call_command("objekt_anschrift_kandidaten", objekt="701", min_hits=2, echt=True, stdout=out)
    assert "<- nicht eindeutig" in out.getvalue() and "0 gesetzt" in out.getvalue()
    obj.refresh_from_db()
    assert not obj.street


def test_standardauswahl_nur_objekte_ohne_strasse_und_fremde_hausnummern_als_hinweis():
    _obj("702", street="Musterweg", house_number="1")
    ohne = _obj("703")
    _doc(ohne, BLOCK)
    _doc(ohne, "Am Beispielpark 7\n12345 Beispielstadt")
    out = StringIO()
    call_command("objekt_anschrift_kandidaten", stdout=out)
    text = out.getvalue()
    assert "Objekt 703" in text and "Objekt 702" not in text
    assert "weitere Hausnummern derselben Strasse: 7" in text
    with pytest.raises(Exception, match="nicht gefunden"):
        call_command("objekt_anschrift_kandidaten", objekt="999")
