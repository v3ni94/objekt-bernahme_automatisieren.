...
| `worker-drosseln` | OCR-Prozesse des laufenden Workers ohne Neustart ändern (Celery `pool_shrink` oder `pool_grow`), Argument Zielanzahl 1 bis 8; wirkt sofort auf die CPU-Last, gilt bis zum nächsten Neustart; dauerhaft über `env-set OCR_PROCESSES=…` und `deploy` | Worker |
