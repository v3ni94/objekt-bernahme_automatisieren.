...
| `env-set` | Betriebswert in `.env` setzen, Argument `SCHLUESSEL=wert`; nur freigegebene Schlüssel (Ressourcen und Parallelität: `REDIS_MEM`, `REDIS_MAXMEMORY`, `OCR_PROCESSES`, `IO_CONCURRENCY`, `NLP_CONCURRENCY`, `GUNICORN_TIMEOUT`, Container-Grenzen), Sicherung `.env.bak`, Eintrag im Deploy-Protokoll; wirksam erst mit `deploy` | `.env` |
