...
- Umfang (Objektnummern): Beim Drive-Lauf begrenzen die Nummern die Objektwurzeln (der Eingangsordner ist dann nicht dabei). Beim Paperless-Lauf fragt die Anwendung je Nummer nur die Dokumente ab, deren Feld MHV Objekt genau diesen Wert trägt (Filter `custom_field_query`, exakter Vergleich); geprüft werden die eingegebene Schreibweise, die Schreibweise ohne führende Nullen und die in der Anwendung geführte Objektnummer (`page_state.scope_values`, Seitenstand je Wert in `scope_index`). Dokumente ohne Feld oder mit anderer Nummer werden nicht gelesen. Ohne Nummern liest der Paperless-Lauf den gesamten sichtbaren Bestand (bei rund 96.000 Dokumenten mit je einem Metadatenabruf entsprechend lange). Geschrieben wird beim echten Lauf in beiden Fällen nur nach `writes_allowed`.
