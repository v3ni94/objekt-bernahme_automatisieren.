...
| `paperless.import_only_with_object` | true | Nur Dokumente mit Feld MHV Objekt übernehmen (direkt in das Objekt); ohne Feld bleibt das Dokument in Paperless, es entsteht kein Eingang. Aus: auch Dokumente ohne Bezug wandern in den Dokumenteneingang |
| `paperless.archive_for_unsupported` | true | Ist das Original für die Verarbeitung ungeeignet (Office-Altformat, HTML; E-Mails `.eml` und `.msg` liest die Kette seit 25.09.2026 selbst und übernimmt sie als Original), wird die PDF-Archivfassung übernommen, die Paperless beim Eingang mit Tika und Gotenberg erzeugt hat; aus: Original übernehmen, es entsteht ein Fall „nicht unterstütztes Format". Bestand nachziehen: Deploy-Aktion `paperless-archivfassung` (Auswahl über die Paperless-Verknüpfung, auch für Drive-Bestandsdateien, die per Prüfsumme verknüpft wurden) |
...
|---|---|---|---|
| `sync-dispatch` | `sync.dispatch_due` | jede Minute | gibt hängende Operationen frei (Heartbeat älter als 30 Minuten) und reiht fällige Operationen nach Priorität ein, die keine Nachricht unterwegs haben: jeder Versand merkt `dispatched_at`, eine Operation gilt 60 Minuten als unterwegs, und höchstens 200 Operationen sind gleichzeitig unterwegs. Wiederholung, Zurückstellung und Freigabe setzen `dispatched_at` zurück. Seit 14.09.2026; zuvor gingen jede Minute die 200 ältesten wartenden Operationen erneut in die Warteschlange, bis Redis voll war |
...
- Umfang (Objektnummern): Beim Drive-Lauf begrenzen die Nummern die Objektwurzeln (der Eingangsordner ist dann nicht dabei). Beim Paperless-Lauf fragt die Anwendung je Nummer nur die Dokumente ab, deren Feld MHV Objekt mit diesem Wert beginnt (Filter `custom_field_query`, Operator `istartswith`), und behält davon nur die, deren erster Zahlenblock der Nummer entspricht (`counters.scope_prefix_skipped` zählt die übrigen, etwa 1330 bei 133). Grund: Paperless 3.1.3 vergleicht `exact` buchstabengenau, Werte wie „133 Musterstraße 1“ aus Speicherpfaden oder Handeingaben fielen heraus, für Nummer 133 blieben 486 von 4.155 Dokumenten (Befund 20.09.2026); geprüft werden die eingegebene Schreibweise, die Schreibweise ohne führende Nullen und die in der Anwendung geführte Objektnummer (`page_state.scope_values`, Seitenstand je Wert in `scope_index`). Dokumente ohne Feld oder mit anderer Nummer werden nicht gelesen. Ohne Nummern liest der Paperless-Lauf den gesamten sichtbaren Bestand (bei rund 96.000 Dokumenten mit je einem Metadatenabruf entsprechend lange). Geschrieben wird beim echten Lauf in beiden Fällen nur nach `writes_allowed`.
- Dokument mit Feld, aber ohne Objekt: Verweist das Feld MHV Objekt auf eine Nummer, zu der in der Anwendung kein aktives Objekt existiert, bleibt das Dokument `out_of_scope` mit dem Grund „kein aktives Objekt mit dieser Nummer angelegt“ (`details.object_field`). Nach dem Anlegen des Objekts genügt ein neuer Bestandslauf für diese Nummer.
...
- Kürzer gewordene Liste: Paperless beantwortet eine Seitennummer jenseits des Endes mit HTTP 404 „Invalid page.“ (Django REST Framework). Verschwinden Dokumente zwischen zwei Schritten aus der Liste (gelöscht oder umgefiltert), behandelt der Lauf das seit 20.09.2026 als Ende der Liste und vermerkt `counters.ended_early_page` und `counters.count_at_end`; Dokumente, die dadurch die Seite gewechselt haben, erfasst der nächste Lauf. Kündigt Seite 1 die Seite dagegen noch an (etwa während eines Paperless-Updates), fällt der Lauf fortsetzbar auf `failed` mit dem Text „Seite n von m nicht abrufbar“ (Bestandslauf 2, 20.09.2026).
...
- Abgleich ab jetzt: Der Knopf „Abgleich ab jetzt“ auf der Paperless-Seite setzt den Cursor `modified_cursor` auf den aktuellen Zeitpunkt (Protokoll `sync.cursor_set`). Ältere Änderungen im DMS werden dann nicht mehr Dokument für Dokument geprüft; der Bestand kommt gesteuert über das Feld MHV Objekt (Massenbearbeitung in Paperless, danach „Paperless jetzt abgleichen“ liest nur Dokumente, deren `modified` nach dem Cursor liegt, also genau die gerade bearbeiteten) oder über einen echten Bestandslauf. Pilotbefund 13.09.2026: 96.265 Dokumente im DMS, davon 96.145 ohne Bezug zu Objektakten; deshalb Standard `paperless.import_only_with_object` an und Cursor ab jetzt.
...
Korrektur nach der Übernahme (Erfahrung Pilot Objekt 82, 22.09.2026): Ein Trockenlauf hält nichts zurück, sobald das Feld in Paperless gesetzt ist, denn jede Feldänderung setzt `modified` neu und die 5-Minuten-Abfrage importiert das Dokument sofort. Wer das Feld in Paperless danach leert oder auf eine andere Nummer stellt, ändert damit die Zuordnung in der Anwendung nicht; `push_meta` würde das Feld beim nächsten Rückschreiben sogar wieder setzen. Der Korrekturweg ist die Deploy-Aktion `paperless-feld-abgleich <objekt>` (Vorschau) beziehungsweise `paperless-feld-abgleich <objekt>+echt` (Kommando `manage.py paperless_feld_abgleich <objekt> [--echt]`): sie liest je Dokument des Objekts mit Paperless-Original das Feld MHV Objekt und meldet `ok`, `leer` (Feld geleert), `anderes_objekt` (bekannte andere Nummer), `unbekannt` (Nummer ohne Objekt), `fehlt` (Dokument in Paperless nicht mehr vorhanden) oder `fehler`. Mit `--echt` gehen geleerte Dokumente per „In anderes Objekt übernehmen“ in das Eingangsobjekt, anders belegte in das genannte Zielobjekt; die Drive-Verschiebung folgt über `file_to_drive`, unbekannte Nummern werden nur gemeldet. Typischer Fehltreffer, den dieser Weg bereinigt: ein Dokument, in dem die Objektadresse nur beiläufig vorkommt (Fahrtkostenabrechnung mit der Objektadresse als Fahrtziel). Jede Übernahme wird als Lernbeispiel festgehalten (geleertes Feld als Ablehnung `reject` des alten Objekts, andere Nummer als Korrektur `correct`). Die Ablehnung wirkt auf die Inhaltszuordnung im Eingang: ein Objekt, das für dieses Dokument bereits manuell verworfen wurde (Feldabgleich oder Ablehnung im Prüfcenter), wird nie erneut automatisch gewählt, sondern nur noch vorgeschlagen (`build_proposal`, Grund „bereits manuell verworfen“). Ohne diese Sperre würde derselbe Adresstreffer das Dokument sofort wieder in das alte Objekt ziehen.

Gegenprobe des Feldimports (seit 22.09.2026, `apps.sync.flows.crosscheck`): Das Feld entscheidet nicht mehr allein. Nach der Texterkennung prüft die Anwendung im Entscheidungsschritt, ob das Dokument eindeutig zum Feldobjekt gehört: `eindeutig` (nur das Feldobjekt mit klarem Bezug, weitere Anschriften eines Eckobjekts zählen dazu), `zweiter_bezug` (ein anderes bekanntes Objekt kommt im Text vor), `schwacher_bezug` (das Feldobjekt nur beiläufig, etwa als Fahrtziel oder Rechnungsanschrift), `kein_bezug` (Text nennt das Feldobjekt nicht, nur Meldung). Nicht eindeutige Fälle gehen an den KI-Schiedsrichter mit dem Feldobjekt als erstem Kandidaten (docs/betrieb/ki-anbindung.md): bestätigt, in das andere Objekt übernommen, bei „kein Objektdokument“ in den Eingang (Fall `ai_not_object`); ohne sichere KI-Antwort bleibt das Dokument, wenn die lokale Regel das Feldobjekt eindeutig wählen würde, sonst geht es mit Prüffall in den Eingang (die KI-Einschätzung wandert mit, kein zweiter Aufruf). Ein umgehängtes Dokument wird im alten Objekt nicht abgelegt. Jede Prüfung setzt `documents.assignment_checked_at` und schreibt das Audit `sync.field_crosscheck` (Befund, andere Objekte, Aktion). Live gilt die Gegenprobe für Dokumente, die ab dem 22.09.2026 angelegt wurden; der Bestand läuft über die Deploy-Aktion `zuordnung-pruefen` (Vorschau je Objekt ohne KI-Aufruf, `echt` markiert Eindeutige und löst Nichteindeutige mit der KI auf, `limit=N` begrenzt die Auflösungen je Lauf, `ohne-ki` löst ohne KI, `details` listet die Dokumente; Kommando `manage.py paperless_zuordnung_pruefen`). Große Läufe in `tmux` starten; die Vorschau zeigt vorab, wie viele Dokumente die KI prüfen würde.
...
### 6.2 Stichprobe der Zuordnung

Zweck: dokumentierte manuelle Kontrolle der aus Paperless übernommenen Dokumente gegen Objekt, Ordner und Dokumentart in Drive (Vorlage E-3 in docs/plan/entscheidungen.md, Nachweis für Abnahmepunkt 3). Das Kommando `manage.py zuordnung_stichprobe` liest nur und ändert nichts; Deploy-Aktion `zuordnung-stichprobe`.

Ablauf:

1. Ziehen: `zuordnung-stichprobe <branch>` ohne Argument zieht je Hauptkategorie 01 bis 06 zufällig 50 abgelegte Dokumente (Status `filed`, nicht gelöscht, kein Eingangsobjekt) mit Paperless-Verknüpfung (Auswahl über die Verknüpfung, nicht über die Dokumentquelle, damit per Prüfsumme verknüpfte Drive-Bestandsdateien mitzählen). Argumente mit `+`: `je=30` (Umfang je Kategorie), `seed=N` (Zufallsauswahl wiederholbar, ohne Angabe das Tagesdatum als JJJJMMTT), `objekt=NR` (nur ein Objekt). Derselbe Seed liefert auf demselben Bestand dieselbe Liste. Die Konsole zeigt nur Zähler je Kategorie (gezogen von Grundgesamtheit), die Verteilung der Herkunft und den Pfad der CSV, keine Titel oder Dateinamen.
2. Liste: CSV unter `/srv/objektakte/exports/stichproben/<JJJJ-MM-TT>_stichprobe.csv` (Semikolon, UTF-8 mit BOM, in Excel oder LibreOffice zu öffnen) mit den Spalten `dokument_id`, `paperless_id`, `objekt`, `kategorie`, `unterordner`, `dokumentart`, `drive_pfad` (Ordnerkette vom Objektordner bis zum Ablageordner aus `drive_nodes`; Ordner mit Status `missing` oder `trashed` sind gekennzeichnet, eine abgebrochene oder mehr als acht Ebenen tiefe Kette beginnt mit `...`; Eigentümer- und Mieterakten erscheinen nicht mit ihrem Ordnernamen, sondern nur als Kennung `Eigentuemerakte #<id>` beziehungsweise `Mieterakte #<id>`, Unterordner der Akte mit dem Katalognamen, damit die Liste keine Personennamen enthält), `herkunft_objektbezug`, `pruefung` (leer), `bemerkung` (leer). Herkunft, soweit ermittelbar: `feldimport` (Feld MHV Objekt beim Pull), `pfad` (Feld aus dem Paperless-Speicherpfad, erkannt über den führenden Zahlenblock des Pfadnamens; braucht die Paperless-Abfrage, sonst bleibt es bei `feldimport`), `ki` (KI-Schiedsrichter oder KI-Gegenprobe), `regel` (automatische Zuordnung aus dem Eingang), `manuell` (Zuordnung im Dokumenteneingang, Übernahme durch einen Nutzer, Feldabgleich nach Handänderung in Paperless), `drive_bestand` (Datei lag schon im Objektordner), `upload`, sonst `unbekannt`. Ist Paperless nicht erreichbar, läuft die Ziehung mit Hinweis durch.
3. Prüfung durch den Sachbearbeiter: je Zeile das Dokument in Paperless (`paperless_id`) beziehungsweise in der Anwendung (`dokument_id`) öffnen und gegen den Ablageort in Drive (`drive_pfad`, Kategorie, Unterordner, Dokumentart) prüfen; Spalte `pruefung` mit `richtig`, `falsch` oder `unklar` füllen, Abweichungen in `bemerkung` (zum Beispiel richtiges Objekt oder richtiger Ordner). Keine Personendaten in die Bemerkung. Die Datei unter demselben Namen zurück nach `/srv/objektakte/exports/stichproben/` legen; speichert Excel unter Windows die Datei als ANSI (Standardformat CSV, Trennzeichen-getrennt), liest die Auswertung sie trotzdem (Rückfall auf cp1252), Dateien in anderer Kodierung werden mit Hinweis abgewiesen.
4. Auswerten: `zuordnung-stichprobe <branch> ergebnis=/data/exports/stichproben/<JJJJ-MM-TT>_stichprobe.csv` (Pfad im Container `worker-io`; das Skript lehnt Pfade außerhalb von `/data/exports/stichproben/` und andere Endungen als `.csv` ab) gibt je Kategorie und gesamt gezogen, richtig, falsch, unklar, offen und die Quote richtig (bezogen auf die geprüften Zeilen) aus, dazu die Dokument-IDs mit Befund `falsch`, und schreibt `exports/stichproben/<JJJJ-MM-TT>_ergebnis.md` als Markdown-Tabelle.
5. Ablage des Ergebnisses: Tabelle aus der Markdown-Datei in docs/betrieb/abgleich-protokoll.md übernehmen (Abschnitt Stichprobe der Paperless-Zuordnung, fortgeschrieben durch den Sachbearbeiter, mit Datum, Seed und Umfang), die ausgefüllte CSV bleibt unter `exports/stichproben/` als Beleg. Dokumente mit Befund `falsch` über das Prüfcenter beziehungsweise „In anderes Objekt übernehmen“ korrigieren; häufen sich Fehler einer Herkunft (etwa `pfad`), ist eine Nachzuordnung des betroffenen Speicherpfads zu prüfen, bevor der Abnahmepunkt als erfüllt gilt.

Umfang: 50 je Kategorie ist der Vorschlag aus E-3 (höchstens 300 Zeilen je Lauf); hat eine Kategorie weniger abgelegte Dokumente, werden alle gezogen. Wiederholte Läufe am selben Tag überschreiben die CSV des Tages nur, solange sie keine Einträge in der Spalte `pruefung` enthält; eine bereits ausgefüllte und zurückgespielte Liste bleibt erhalten, der Lauf bricht mit Hinweis ab (Zusatzziehung dann mit `csv=<pfad>` in einen anderen Dateinamen, absichtliches Ersetzen mit `ueberschreiben`). Ein anderer Seed ergibt eine neue Auswahl. Das Kommando läuft im Dienst `worker-io`, weil der Web-Container `exports` nur lesend einbindet.
...
| Redis voll (`OutOfMemoryError: command not allowed when used memory > 'maxmemory'` in den Logs von `worker`, `worker-io`, `beat`) | alle Worker starten in Schleife neu, Beat kann keine Aufgaben senden, keine Verarbeitung mehr, HTTP 500 bei Aktionen, die eine Redis-Sperre setzen | Redis läuft mit `noeviction`: Schreiben ist gesperrt, Daten bleiben erhalten (AOF) | Deploy-Aktion `redis-status` (Speicher, Nachrichten je Warteschlange); Grenze mit `env-set REDIS_MEM=…` und `env-set REDIS_MAXMEMORY=…` anheben und `deploy` ausführen; Ursache der Nachrichtenflut prüfen (14.09.2026: Mehrfachversand offener Jobs bei jedem Eingang, seither Versandzeitpunkt je Job und Operation) |
