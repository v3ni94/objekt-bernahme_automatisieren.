...
- Beide Dateien müssen vor `docker compose up` vorhanden sein. `scripts/deploy.sh` legt fehlende Dateien als leere Platzhalter mit Rechten 0444 und Eigentümer root an (über `sudo -n install`, ersatzweise direkt, wenn das Verzeichnis beschreibbar ist). Gelingt beides nicht, bricht das Deployment vor dem Bau der Container mit dem Hinweis ab, welcher Befehl einmalig als root auszuführen ist (`install -m 0444 -o root -g root /dev/null /srv/objektakte/secrets/<name>`). Leer bedeutet: Anbindung aus (`configured()` ist falsch, es entsteht kein Client).
