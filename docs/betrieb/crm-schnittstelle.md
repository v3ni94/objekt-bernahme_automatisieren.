...
| `CRM_WEBHOOK_SECRET` | Secret-Datei über `CRM_WEBHOOK_SECRET_FILE`, lokal auch als Variable | gemeinsames Geheimnis für die Signatur; leer bedeutet aus |
| `CRM_WEBHOOK_TIMEOUT_S` | `.env`, Standard 10 | Zeitgrenze je Versuch in Sekunden |

Offener Punkt Betrieb: `docker-compose.yml` bindet noch keine Secret-Datei für das Webhook-Geheimnis ein. Vor dem Einschalten ist analog zu `paperless_webhook_token` ein Secret `crm_webhook_secret` unter `/srv/objektakte/secrets/` anzulegen und für `web` und `worker-io` mit `CRM_WEBHOOK_SECRET_FILE: /run/secrets/crm_webhook_secret` einzubinden. Bis dahin bleiben die Webhooks aus, auch wenn eine URL gesetzt ist.
