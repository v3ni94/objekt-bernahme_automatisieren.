...
- KI-Einschätzung im Fall: Konnte die Anwendung nicht eindeutig zuordnen, steht auf der Falldetailseite die Einschätzung der KI (Objektdokument ja oder nein, vorgeschlagenes Objekt mit Konfidenz, Rolle weiterer Anschriften, Begründung). Ein sicherer KI-Vorschlag wurde bereits übernommen (Fall `ai_auto`, erledigt); „kein Objektdokument“ (Fall `ai_not_object`) bleibt im Eingang und wartet auf die Entscheidung, ob das Dokument in ein Objekt, die Belegprüfung oder den Papierkorb gehört.
- Fälle aus der Gegenprobe (Kontext `crosscheck` mit Herkunftsobjekt und weiteren Objekten): Das Dokument kam über das Paperless-Feld in ein Objekt, nannte aber ein zweites Objekt oder das Feldobjekt nur beiläufig, und weder KI noch lokale Regel waren sicher. Entscheidung wie bei jedem Zuordnungsfall; das Herkunftsobjekt steht als Vorschlag oder in den Kandidaten.
