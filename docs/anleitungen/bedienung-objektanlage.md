...
Jahresordner der Buchhaltung: `03_Buchhaltung` erhält seit dem 24.09.2026 Jahresordner `JJJJ` (Wunsch der Geschäftsführung). Die Ablage legt Gesamtjahresabrechnung und Gesamtwirtschaftsplan nach ihrem Abrechnungs- oder Wirtschaftsjahr, Kontoauszüge und Belege nach dem Beginn des Zeitraums oder dem Dokumentdatum in den Jahresordner; ohne erkennbares Jahr bleibt das Dokument flach in `03_Buchhaltung`, bis im Prüfcenter ein Jahr erfasst wird. Jahresordner entstehen erst bei der ersten Ablage eines Jahres (nicht durch den Ordnerabgleich), von Hand angelegte Ordner `2024` werden per Namen übernommen, der Abgleich registriert sie. Das Prüfcenter und die Sammelaktion zeigen den Zielpfad mit Jahr (`03_Buchhaltung/2026`). Bereits flach abgelegte Dokumente bringt die Deploy-Aktion `buchhaltung-jahresordner` (Vorschau, dann `echt`) in die Jahresordner; dabei wird nur verschoben, nie gelöscht.
...
Was nicht passiert: Es wird nichts gelöscht. Die Quellordner bleiben stehen (nach der Übernahme leer). Verknüpfungen werden übersprungen, weil das Original an anderer Stelle liegt. Temporär- und Systemdateien (`*.tmp`, `~$…`, `Thumbs.db`, Muster in `drive.takeover_ignore_patterns`) werden nicht übernommen. Alte Word- und Excel-Dateien (`.doc`, `.xls`, `.pub`) kann die Anwendung nicht lesen; sie landen als „nicht unterstütztes Format“ im Review Center und werden dort von Hand zugeordnet. E-Mails (`.eml`, `.msg`) liest die Anwendung seit 25.09.2026 als Textseite (Kopfzeilen, Textkörper, Liste der Anhänge mit Größe; Anhänge selbst werden nicht gelesen, ein Text nur als RTF bleibt ungelesen) und klassifiziert sie wie Office-Dateien. Eine Datei, die bereits einem Objekt zugeordnet ist, zeigt die Seite mit dem Hinweis „in Objekt …“ und übernimmt sie nicht erneut.
...
8. Automatisches Nachräumen: Sobald ein Verarbeitungslauf des Objekts abgeschlossen ist und kein Job mehr offen ist, prüft die Anwendung den Objektordner und alle aufgearbeiteten Quellordner. Ordner, die nicht zur Sollstruktur gehören und leer sind (keine Dateien, keine nicht leeren Unterordner), sowie nicht registrierte Temporär- und Systemdateien gehen in den Drive-Papierkorb, Unterordner vor Elternordnern und erst nach einer Live-Prüfung, dass der Ordner wirklich leer ist. Alles mit Dokument bleibt liegen und schützt seinen Ordner, auch Dubletten und Dateien in Prüfung; Strukturordner, Objektwurzel und Eingangsordner sind tabu. Ein leer gewordener Quellordner verschwindet aus der Altbestand-Tabelle. Dubletten bleiben dem „Aufräumen“ von Hand vorbehalten. Schalter `drive.auto_cleanup_enabled`, Protokoll `drive.auto_cleanup` und `drive.trash` je Bewegung. Nichts wird endgültig gelöscht.
...
„Alles aufarbeiten“ (Knopf oberhalb der Tabelle, Recht objects.write): arbeitet alle Quellordner mit Objekt nacheinander im Hintergrund auf, wie „Aufarbeiten“ je Zeile. Der Stand (Ordner erledigt, Dateien übernommen, übersprungen, gerade bearbeiteter Ordner, Fehler) steht oberhalb der Tabelle; ein Fehler eines Ordners hält die anderen nicht an, ein zweiter Klick während des Laufs wird abgewiesen. Bereits übernommene Dateien werden übersprungen, deshalb ist ein erneuter Sammellauf jederzeit unschädlich. Ebenso über den Deploy-Workflow (Aktion `altbestand-aufarbeiten`, Argument `echt`).

„Verarbeitung für alle Objekte starten“ (Objektliste, Recht documents.ingest): reiht für jedes aktive Objekt mit offenen Dokumenten (registriert, gehasht, Texterkennung fertig oder fehlgeschlagen) einen Nachlauf ein. Die Läufe starten nacheinander nach `processing.max_parallel_objects`; Objekte mit wartendem oder laufendem Lauf werden ausgelassen. Deploy-Aktion `verarbeitung-alle`.

Massenanlage (einmalig, Admin über den Deploy-Workflow, Aktion `altbestand-objekte`): legt für jede Nummer der Altbestand-Tabelle ohne Objekt ein Objekt an. Bezeichnung und Verwaltungsart kommen aus dem Objektregister (`db/seeds/objektregister.csv`, Immoware24-Export 01.07.2026), sonst aus dem Ordnernamen mit Vorgabe WEG; jedes so angelegte Objekt trägt in den Notizen den Hinweis „Stammdaten bitte nachpflegen“ samt Registerstand (aktiv, nur Abrechnung, abgegeben). Quellordner mit der Nummer werden gebunden, der Objektordner in Drive entsteht wie bei der Anlage von Hand. Fehlt die Anschrift, legt der Ordnerabgleich seit 14.09.2026 einen vorläufigen Objektordner aus Nummer und Bezeichnung an (`drive.object_folder_fallback_pattern`); er wird über die Nummer erkannt, der Name kann nach der Nachpflege in Drive angepasst werden. Ohne Argument zeigt die Aktion nur die Vorschau; „Aufarbeiten“ bleibt je Ordner ein eigener Schritt.
...
6. Sollbestand der Stammakte (Checkliste der Geschäftsführung vom 12.09.2026): Zusätzlich zu den Prüfpunkten aus CR 12 bewertet die Anwendung je Objekt die empfohlenen Unterlagen der Stammakte, getrennt nach Grundbestand (alle Verwaltungsarten), WEG, Mietverwaltung und SEV. Eine Position gilt als vorhanden, sobald ein bestätigtes Dokument einer zugehörigen Dokumentart im Objekt liegt; ein Dokument in Prüfung zählt als teilweise. Diese Positionen haben Empfehlungscharakter: sie zählen nicht in den Erfüllungsgrad, stehen als eigene Zeile „Sollbestand Stammakte“ in der Bewertung und werden erst in die Nachforderung aufgenommen, wenn sie in der Zeile angehakt sind. Statusangaben für die Übersteuerung: vorhanden und geprüft; vorhanden, aber unvollständig oder Aktualität ungeklärt (mit Grund); nicht anwendbar. Nicht zutreffende Positionen bitte als nicht anwendbar kennzeichnen, damit die Liste der offenen Punkte aussagekräftig bleibt. Der Sollbestand umfasst 54 Prüfpositionen: 40 für alle Verwaltungsarten (Objektstammdaten bis Übernahme), 9 nur WEG (Teilungserklärung mit Nachträgen, Aufteilungsplan, Sondernutzungsrechte und Kostenverteilung, Beschlusssammlung, Versammlungsunterlagen, Umlaufbeschlüsse, bauliche Veränderungen, Verwaltervertrag der Vorverwaltung, Gerichtsverfahren), 4 nur Mietverwaltung (Einheitenstammblätter, Inventarverzeichnis, Zuständigkeitsregelung, Belegungsbindung) und 1 nur SEV (Kontaktblatt der WEG-Verwaltung). Angehakte Positionen erscheinen im Nachforderungsschreiben unter der Gruppe „Sollbestand der Stammakte“. Als Nachweis gelten die 101 Dokumentarten der Stammakte (je Unterordner, zum Beispiel Baugenehmigung, Schließplan, Wartungsvertrag, Übergabeprotokoll der Vorverwaltung), die Stufe 1 an Titel und typischen Formulierungen erkennt; Verträge werden nach Art unterschieden (Hausmeister-, Reinigungs-, Winterdienst-, Gartenpflege-, Wartungsvertrag), die allgemeine Art Dienstleistervertrag bleibt Auffangregel.
