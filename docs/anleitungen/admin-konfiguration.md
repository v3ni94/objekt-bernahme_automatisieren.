...
| `drive.auto_cleanup_enabled` | boolean | `true` | Nach jedem abgeschlossenen Verarbeitungslauf eines Objekts (kein Job mehr offen) leere Ordner außerhalb der Sollstruktur und nicht registrierte Temporär- und Systemdateien (`drive.takeover_ignore_patterns`) im Objektordner und in aufgearbeiteten Altbestand-Ordnern in den Drive-Papierkorb verschieben; Strukturordner, Objektwurzel, Eingangsordner, Dateien mit Dokument (auch Dubletten) und Google-Dokumente bleiben; ein leerer Quellordner verschwindet aus der Altbestand-Tabelle; Protokoll `drive.auto_cleanup` | Anforderung 13.09.2026 | boolean |
...
| `drive.takeover_ignore_patterns` | list | `["*.tmp", "*.TMP", "~$*", "Thumbs.db", "desktop.ini", ".DS_Store", "*.lnk"]` | Dateinamenmuster, die bei der Übernahme aus Drive übersprungen werden (Temporär- und Systemdateien). Dieselben Muster gelten beim Aufräumen nach der Aufarbeitung: nicht registrierte Dateien mit passendem Namen gehen als Temporär- oder Systemdatei in den Papierkorb (Vorschau vorher prüfen) | Befund 11.09.2026 | array |
...
| `drive.max_requests_per_second` | integer | `5` | Clientseitige Ratenbegrenzung als Token-Bucket je Adapteraufruf (`get_adapter` je Aufgabe): begrenzt Bursts innerhalb einer Aufgabe, nicht die Summe über Threads oder Prozesse; eine Anhebung erhöht den Gesamtdurchsatz daher nicht (Versuch 14.09.2026 mit 8, zurückgenommen) | ANNAHME A15 | min 1, max 50 |
...
| `paperless.archive_for_unsupported` | boolean | `true` | Ist das Original aus Paperless für die Verarbeitung ungeeignet (Office-Altformat, HTML; E-Mails `.eml` und `.msg` liest die Kette seit 25.09.2026 selbst), wird die PDF-Archivfassung übernommen, die Paperless beim Eingang erzeugt hat; aus: Original übernehmen und Fall „nicht unterstütztes Format" | Bestandsbefund 25.09.2026 (6.527 Fälle „nicht unterstütztes Format" aus Paperless) | boolean |
...
| `sync.assignment_ai_enabled` | boolean | `true` | KI-Schiedsrichter der Objektzuordnung: greift, sobald die Zuordnung eines Eingangsdokuments nicht eindeutig ist und Kandidaten vorliegen; sicherer Kandidat wird übernommen, kein Objektdokument bleibt im Eingang mit Fall, sonst Prüffall mit KI-Einschätzung; wirkt nur mit freigegebenem Anbieter und Preisliste | Entscheidung 22.09.2026 | boolean |
| `sync.assignment_ai_min` | decimal | `0.85` | Mindestkonfidenz der KI für eine automatische Objektzuordnung; darunter bleibt ihr Kandidat Vorschlag im Prüffall | Entscheidung 22.09.2026 | min 0.5, max 1 |
