...
3. Im Google-Dialog mit **`ablage@muellerhv.de`** anmelden, nicht mit dem persönlichen Konto. Nach dem Callback ermittelt die Anwendung das autorisierende Konto über die Drive-Schnittstelle (`about.get`, funktioniert mit dem Drive-Bereich allein), prüft die E-Mail gegen `DRIVE_ACCOUNT_EMAIL` und lehnt jedes andere Konto ab (Audit `drive.authorize` mit Ergebnis).
...
| T13 | Login und Rollen | Neuer Admin ohne TOTP: nach Login nur Einrichtungsseite erreichbar; neuer Sachbearbeiter ohne TOTP: Anmeldung ohne Umleitung (Pflichtrollen aus `security.mfa_required_roles`, seit 12.09.2026 nur `admin`); nach TOTP „Dieses Gerät merken“: Cookie `mfa_trusted` mit Secure, HttpOnly, SameSite Lax und 90 Tagen, zweite Anmeldung ohne Code, nach TOTP-Zurücksetzung wieder mit Code; Sachbearbeiter ruft Konfigurationsseite auf; Sachbearbeiter versucht `review.dismiss_object_case` | Einrichtung nur für Admin erzwungen; Gerät gemerkt und nach Zurücksetzung vergessen; Zugriff verweigert, `audit_events` enthält `auth.denied` | Bildschirmfoto, Abfrage | M1, Erweiterung M7 und 12.09.2026 |
