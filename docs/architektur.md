...
| L Synchronisation | document_versions, sync_links, sync_operations, sync_cursors, sync_inventory_runs, sync_inventory_items, sync_assignment_examples, sync_assignment_rules | Dateiversionen, Verknüpfungen zu Paperless-ngx und Drive je System und Rolle, dauerhafte Operationsliste, Cursor der Änderungsprotokolle, Bestandsmanifest, Lernbeispiele und Regeln der Objektzuordnung (Abschnitt 14, Ergänzung vom 12.09.2026) |
...
| drive_nodes | object_id, node_kind (data_root, object_root, main_folder, subfolder, owner_file_folder, owner_file_subfolder, tenant_file_folder, tenant_file_subfolder, list_file, year_folder), Fremdschlüssel auf Kategorie, Unterordner, Akte, list_type, list_format, year (Jahresordner unter 03_Buchhaltung, seit 24.09.2026), drive_file_id, drive_name, expected_name, created_by_app, status (active, missing, trashed), generierte position_key (mit Jahr) | Jede Zeile hängt an einer Fachentität, nie an einem Namen. Persistenter Folder-ID-Cache. |
...
| document_versions, sync_links, sync_operations, sync_cursors, sync_inventory_runs, sync_inventory_items, sync_assignment_examples, sync_assignment_rules | Dateiversion je Inhaltsstand; Verknüpfung je Dokument, System (paperless, drive) und Rolle (original, archive, snapshot, index_stub) mit externer Kennung, Prüfsummen, zuletzt abgeglichenem Stand (synced_fields) und Zustand; Operationen mit op_key, Art, Status, Versuchen, Wartezeit und Fehlertext; Cursor je System und Name; Bestandsläufe mit Manifest je Datei und Disposition; Lernbeispiele (confirm, correct, reject) und versionierte Regeln der Objektzuordnung | Ein externes Objekt gehört genau einem Dokument; Operationen sind über op_key idempotent; Löschungen werden nie gespiegelt, nur als Zustand geführt; Regeln entstehen erst aus mehreren Bestätigungen (Abschnitt 14). |
...
| 3 | analyze_pages (ocr) | Formatweiche (PDF; Bildformate über img2pdf; Office-Dateien, E-Mails und HTML ohne OCR mit Textextraktion; Office-Altformate `.doc`, `.xls`, `.rtf`, `.odt`, `.ods` als `office_legacy` erkannt und über LibreOffice (`apps.pipeline.office_convert`, nur im Worker-Image, `SOFFICE_BIN`, Zeitlimit `processing.office_convert_timeout_s`, Kindprozess in eigener Prozessgruppe, die bei Zeitüberschreitung vollständig beendet wird) im Arbeitsverzeichnis nach `converted.pdf` gewandelt, danach Analyse wie PDF mit Seitenbildern, abgelegt bleibt das Original; Präsentationen `.ppt` und `.odp` bleiben `unsupported`, weil das Worker-Image kein `libreoffice-impress` enthält (Paperless-Archivfassung als Ersatz); ohne LibreOffice oder bei gescheiterter Umwandlung Fall „nicht unterstütztes Format" mit Notiz „Umwandlung nicht verfügbar" bzw. „Umwandlung fehlgeschlagen", `content_checked=true` und `detected_kind=office_legacy` im Fallkontext, Wiederaufnahme über `formate_wiederaufnehmen --gruppe office` (auch für Altformate, die nur die Inhaltsprüfung erkannt hat, etwa `.tmp` mit RTF-Inhalt); sonst 06_Sonstiges/02_Manuelle_Pruefung mit Review-Fall). Dateien ohne Endung oder mit Temporär-Endung (`.tmp`, `.herunterladen`, `.indir`, `.hed`, `.part`, `.crdownload`) prüft `apps.pipeline.sniff` am Inhalt (Magic Bytes, ZIP- und OLE-Container, E-Mail-Kopfzeilen, HTML); die erkannte Art wird verwendet und in der Fallnotiz vermerkt, der Fall „nicht unterstütztes Format" trägt dann `content_checked=true` im Kontext (26.09.2026). Ergibt die Inhaltsprüfung eine maßgebliche Endung, erhält das Dokument diesen Namen und MIME-Typ (`Liste.hed` mit Excel-Inhalt wird `Liste.xlsx`, `Rechnung.pdf.tmp` wird `Rechnung.pdf`), die lokale Kopie heißt `original.<endung>`, Protokoll `document.rename_by_content`; die Ablage benennt eine Drive-Bestandsdatei entsprechend um (`drive.rename`), die Paperless-Übertragung wird erneut vorgemerkt, wenn die erste an der Temporär-Endung gescheitert war, und die Listenerkennung greift wie bei regulär benannten Dateien. Je Seite Textebenen-Vorprüfung mit pypdfium2 oder PyMuPDF: digital, wenn mindestens digital_min_chars Zeichen, Anteil alphanumerischer Zeichen über digital_alnum_ratio und die Seite nicht nur aus einem seitenfüllenden Bild besteht. Ergebnis: Liste digitaler und OCR-pflichtiger Seiten; rein digitale Dateien starten ocrmypdf nicht. |
...
### 7.5a Nachräumen nach der Verteilung (13.09.2026)

apps.drive.auto_cleanup läuft nach jedem abgeschlossenen Verarbeitungslauf eines Objekts (maybe_finish_run, Aufgabe drive.auto_cleanup in der Warteschlange io; in Tests sofort) und, seit ein Lauf bereits endet, wenn nur noch Ablagejobs offen sind (14.09.2026), erneut mit der letzten Ablage des abgeschlossenen Laufs (jobs._after_filing, Auslöser filing), nur ohne offene Jobs und unter derselben Objektsperre wie Übernahme und Aufräumen. Bäume: Objektordner (Wurzel selbst nie) und aufgearbeitete Altbestand-Quellordner (Wurzel darf, dann Zeile entfernt, drive.takeover_source_prune). Je Baum ein Durchlauf mit walk: Dateien mit Dokument (jeder Status), Google-Dokumente und unbekannte Dateien bleiben und schützen ihren Ordner; nicht registrierte Dateien nach drive.takeover_ignore_patterns gehen in den Papierkorb; danach leere Ordner außerhalb der Sollstruktur (registrierte drive_nodes, Objektwurzeln und Eingangsordner sind geschützt), Kinder vor Eltern und mit Live-Nachprüfung unmittelbar vor dem Papierkorb. Dubletten bleiben dem Aufräumen von Hand vorbehalten (Bestätigung, Step-up). Protokoll drive.trash je Bewegung (kind junk oder empty_folder, trigger) und drive.auto_cleanup je Lauf mit Zählern; Schalter drive.auto_cleanup_enabled. Nichts wird endgültig gelöscht.
...
| Synchronisation mit Paperless und Drive verwalten (Verbindungstest, Kennzeichnung, Eingang einrichten, Bestandsläufe, Operationen), Abschnitt 14 | sync.manage | ja | nein |
| Dokumenteneingang bearbeiten (Objektzuordnung, Konflikte, Dublettenverdacht), Abschnitt 14 | inbox.work | ja | ja |
...
│   ├── betrieb/                  Ausgefüllte Ergebnisblätter, Protokolle und Nachweise: serverbefund.md, performance-entscheidung.md, performance-bericht.md, deployment-test.md, restore-protokoll.md, oauth-nachweis.md, abgleich-protokoll.md, wartung.md; ausführliche Fassungen deployment.md, runbook.md, google-oauth.md, paperless-sync.md
│   ├── plan/                     status.md, entscheidungen.md, berichte/ (Meilenstein- und Prüfpunktberichte)
│   └── anleitungen/              bedienung-review-center.md, bedienung-objektanlage.md, bedienung-dokumenteneingang.md, admin-konfiguration.md (Kurzanleitungen Review Center, Objektanlage, Dokumenteneingang, Import, Listen, Nachforderung, Admin-Konfiguration)
├── src/
...
│       ├── sync/                 Synchronisation Paperless-ngx und Drive (Abschnitt 14): Paperless-Client und Fake, Konfiguration, Operationsliste, Abläufe (flows), Drive-Änderungsprotokoll, Bestandslauf, Objektzuordnung mit Lernfunktion (assignment), Webhook, Verwaltungsseite, Dokumenteneingang
...
## 14. Synchronisation Paperless-ngx und Google Drive

Ergänzung vom 12.09.2026 (Auftrag Synchronisation Paperless-ngx und Google Drive). Umsetzung im Modul src/apps/sync mit additiven Migrationen (documents.uuid, objects.is_system_inbox, acht neue Tabellen); Betriebsdokumentation in docs/betrieb/paperless-sync.md, Bedienung in docs/anleitungen/bedienung-dokumenteneingang.md, Konfigurationsschlüssel in docs/anleitungen/admin-konfiguration.md. Quelle der Wahrheit für Namen, Schwellen und Abläufe ist der Code; dieses Kapitel beschreibt Zielbild, Datenmodell und Regeln.

### 14.1 Zielbild und Rollen der drei Systeme

| System | Rolle | Führt | Die Anwendung schreibt dort |
|---|---|---|---|
| Anwendung (Objektakte) | einziger Koordinator | fachliches Dokument mit unveränderlicher UUID, Objektbezug, Klassifikation, Ablageort, Operationsliste, Konflikte und Zuordnungsvorschläge | alle eigenen Tabellen; jede Entscheidung über Übertragung, Übernahme und Konflikt fällt hier |
| Google Drive | Ablage des Originals in der Objektstruktur (Abschnitt 7) | Dateiinhalt, Ordnerstruktur, Papierkorb | nur technische Kennzeichen (appProperties mhv_uuid, sha256, document_id, object_id, mhv_gen) und Verschiebungen der Ablage; nie Überschreiben, nie Löschen |
| Paperless-ngx | Dokumentenmanagement mit Volltextsuche, Tags, Notizen und Scan-Eingang | eigene OCR, Tags, Notizen, Titel nach dem Upload | Zweitkopie der Datei (oder Exportfassung oder Indexbeleg), ein Kennzeichnungs-Tag und vier benutzerdefinierte Felder (Dokument-UUID, Objekt, Zuordnung, Drive-Link) |

Grundsätze: Paperless wird ausschließlich über die REST-API angesprochen (Token im Authorization-Header, API-Version über Accept), nie über Datenbank, Dateisystem oder Consume-Ordner. Paperless-Workflows lösen nur aus (Webhook), sie schreiben nichts in die Anwendung. Es gibt keine Löschspiegelung, keine Überschreibung von Originalen in Drive und keine automatische Konfliktauflösung nach Zeitstempel. In Paperless landen keine Personendaten: das Feld Objekt trägt Objektnummer, Ort und Straße, das Feld Zuordnung einen Statustext, der Drive-Link verweist auf die Datei mit den Berechtigungen von Drive.

### 14.2 Identität und Datenmodell

Identität: documents.uuid (UUIDField, eindeutig, nicht editierbar, beim Anlegen vergeben) ist die Kennung des fachlichen Dokuments über alle Systeme hinweg. Bei der Übernahme in ein anderes Objekt (apps.documents.transfer.transfer_document) entsteht eine neue documents-Zeile im Zielobjekt (source moved_in); die UUID wandert auf die Nachfolgezeile, die Altzeile erhält eine neue UUID und den Status moved_out mit Verweis (duplicate_of_document_id). Externe Verknüpfungen und Dateiversionen werden im Hook on_document_transferred auf die Nachfolgezeile umgehängt. documents.source kennt zusätzlich den Wert paperless. objects.is_system_inbox kennzeichnet das technische Eingangsobjekt; der Manager ManagedObject.active blendet es aus, sodass es in keiner Objektliste, keinem Ordnerabgleich, keiner Liste und keiner Vollständigkeitsbewertung erscheint.

| Tabelle (Modell) | Schlüsselinhalte | Regeln |
|---|---|---|
| document_versions (DocumentVersion) | document_id, version_no, sha256, size_bytes, mime_type, original_name, source_system (paperless, drive, app), source_external_id, source_version, label, created_by | Unique (document, version_no) und (document, sha256). Eine neue Version entsteht nur bei echter Inhaltsänderung, nicht bei Umbenennung oder Objektkorrektur. |
| sync_links (ExternalLink) | document_id, version_id, system (paperless, drive, app), role (original, archive, snapshot, index_stub), external_id, external_version, parent_external_id, checksum_sha256, checksum_md5, mime_type, size_bytes, remote_modified_at, last_seen_at, last_synced_at, synced_fields (zuletzt gemeinsam abgeglichener Stand), state, state_reason, tombstone_at, tombstone_by | Ein externes Objekt (system, external_id, role) gehört genau einem Dokument; ein Dokument hat je System und Rolle höchstens eine Zeile. Eine fremde Zeile mit derselben externen Kennung wird nie stillschweigend übernommen (Operation blockiert). Zustandsmodell in 14.3. |
| sync_operations (SyncOperation) | op_key (eindeutig), kind (paperless_push, paperless_await_task, paperless_push_meta, paperless_pull, paperless_pull_meta, paperless_index_stub, drive_set_props, drive_export_snapshot, drive_register, assign_object), system, document_id, link_id, source_system, source_revision, desired_state, payload, status (pending, running, done, failed, blocked, cancelled, skipped), priority, attempt_count, max_attempts, next_attempt_at, dispatched_at, locked_by, locked_at, heartbeat_at, started_at, finished_at, last_error, blocked_reason, result, created_by | Dauerhafte Operationsliste; op_key ist der Idempotenzschlüssel aus Art, Dokument-UUID und Quellrevision. Reservierung per UPDATE pending nach running; Wiederholung mit zunehmender Wartezeit; nicht lösbare Operationen bleiben sichtbar (14.5). |
| sync_cursors (SyncCursor) | system, name, value, meta | Unique (system, name). Genutzt: paperless connection (Verbindungsbefund mit Serverversion, API-Version, Funktionen, Tag-ID, Feld-IDs), paperless modified_cursor und last_poll, drive page_token (mit drive_id) und last_changes_poll. |
| sync_inventory_runs (InventoryRun) | kind (paperless, drive), dry_run, status (planned, running, paused, done, failed, aborted), scope, cursor_before, page_state, counters, started_by, started_at, finished_at, error_message | Wiederaufnehmbarer Bestandslauf in Paketen; counters.complete nur bei Status done ohne Einträge pending oder error (14.8). |
| sync_inventory_items (InventoryItem) | run_id, system, external_id, parent_external_id, name, mime_type, size_bytes, checksum_sha256, checksum_md5, remote_modified_at, object_id, document_id, operation_id, disposition (pending, link_existing, import_new, duplicate, export_snapshot, index_stub, unsupported, out_of_scope, in_progress, error), target_external_id, error_message, details | Manifest: eine Zeile je gefundener Datei, Unique (run, system, external_id); wird vor der Fortschrittsbestätigung des Laufs geschrieben. |
| sync_assignment_examples (AssignmentExample) | document_id, kind (confirm, correct, reject), source (human, auto_verified, import_unverified), previous_object_id, target_object_id, proposed_object_id, decided_by, decided_at, features, evidence, rule_version, review_case_id, is_active | Lernbeispiel der Objektzuordnung; negative Beispiele bleiben erhalten (14.10). |
| sync_assignment_rules (AssignmentRule) | code, version, kind (feature_combo), conditions, object_id, is_active, created_from, created_by, hit_count, last_hit_at, deactivated_at, deactivated_by, notes | Unique (code, version); Regeln sind versioniert und abschaltbar, entstehen nur aus mehreren Bestätigungen und sortieren bestätigte Altdokumente nie um. |

Dateien liegen nie als BLOB in der Datenbank. Konflikte und Zuordnungsvorschläge sind Review-Fälle (review_cases) mit den neuen Fallarten object_assignment (Unterfälle proposal, no_candidate, from_paperless, auto) und sync_conflict (Unterfälle in 14.9).

### 14.3 Zustandsmodell der Verknüpfung

| Zustand (sync_links.state) | Bedeutung | Gesetzt durch |
|---|---|---|
| pending | vorgemerkt, noch kein externer Stand | Vorgabewert beim Anlegen |
| linked | Verknüpfung hergestellt, Felder noch nicht abgeglichen | vorhandene Kopie per UUID oder Prüfsumme gefunden; Dokument aus Paperless übernommen |
| synced | Stand beider Seiten stimmt überein | Upload abgeschlossen, Felder geschrieben, Drive-Kennzeichen gesetzt, bekanntes Dokument ohne Abweichung geprüft, Konflikt mit keep_local entschieden |
| changed_remote | Inhalt extern geändert | Prüfsumme in Paperless oder MD5 in Drive weicht vom bekannten Stand ab (Konfliktfall) |
| conflict | Objektbezug extern geändert | Feld Objekt in Paperless nennt ein anderes Objekt (Konfliktfall object_changed_remote) |
| missing | extern nicht gefunden | 404 in Paperless, Entfernen in Drive oder kein Zugriff mehr (Konfliktfall) |
| trashed | im Papierkorb | deleted_at in Paperless, trashed in Drive (Konfliktfall) |
| no_access | kein Zugriff | im Modell vorgesehen; die aktuellen Abläufe führen fehlenden Zugriff unter missing |
| tombstone | Löschung bestätigt | Entscheidung tombstone im Konfliktfall; die Kennung wird beim Eingang übersprungen und nie erneut übernommen |

Übergänge aus einer Entscheidung: keep_local setzt synced (bei Papierkorb oder Löschung bleibt trashed oder missing stehen und wird als gesehen vermerkt), tombstone setzt tombstone mit Zeitpunkt und Person, reupload löscht die Verknüpfung und reiht eine neue Übertragung ein, apply_remote_object und push_local_object setzen synced. Alle Übergänge schreiben state_reason; Konfliktentscheidungen stehen zusätzlich im Audit (sync.conflict_resolved, sync.tombstone).

### 14.4 Prioritätsregeln je Feld

| Feld | Führendes System | Verhalten |
|---|---|---|
| Dateiinhalt | Drive (Original) | wird nie überschrieben; abweichender Inhalt in Paperless oder Drive ergibt einen Konfliktfall |
| Volltext, OCR-Ergebnis, Notizen in Paperless | Paperless | werden nie geschrieben; Notizen werden nur gelesen |
| Tags | Paperless | nur der eigene Tag (paperless.tag_name) wird ergänzt, fremde Tags bleiben |
| Titel | Paperless nach dem Upload | beim Upload aus dem Dateinamen gesetzt; spätere Änderungen werden als Hinweis in synced_fields gespeichert und lösen keine Umbenennung aus |
| Dokument-UUID | Anwendung | Feld Dokument-UUID; Identität über beide Systeme, wandert bei Objektübernahme mit |
| Objektbezug | Anwendung | Feld Objekt (Nummer, Ort, Straße); eine Abweichung in Paperless wird Konflikt (Dokument in einem Objekt) oder Vorschlag (Dokument im Eingang), nie stillschweigend übernommen |
...
| Drive-Link | Anwendung | Feld Drive-Link auf die Datei; Zugriff nach Drive-Berechtigung |
| Kategorie, Unterordner, Dateiname, Ablageort | Anwendung | Ergebnis von Pipeline und Drive-Ablage; aus Paperless nicht änderbar |
| Übrige benutzerdefinierte Felder | Paperless | unberührt; geschrieben werden nur die vier Felder der Anwendung und nur bei geändertem Sollwert (Vergleich mit synced_fields.app_fields) |

### 14.5 Ablauf Ausgang: Anwendung nach Paperless und Drive

Einhängepunkte (apps.sync.hooks, aus apps.pipeline.tasks und apps.documents.transfer aufgerufen, jeder Aufruf gegen Ausnahmen gesichert, damit die Pipeline nie an der Synchronisation scheitert):

| Schritt | Auslöser | Wirkung |
|---|---|---|
| 1 | Hash eines Dokuments (Job hash, on_document_hashed) | Operation paperless_push mit Schlüssel aus Art, UUID und SHA-256, wenn die Quelle nicht Paperless ist, Schreiben für das Objekt erlaubt ist (writes_allowed) und noch keine Paperless-Verknüpfung besteht |
| 2 | paperless_push | Vorprüfung per UUID-Feldsuche (custom_field_query, falls der Server sie bietet): vorhandene Kopie wird verknüpft statt erneut hochgeladen. Weiche nach Dateiart: natives Google-Dokument ergibt drive_export_snapshot (Exportfassung, Rolle snapshot); Endung außerhalb der Liste PAPERLESS_SUFFIXES oder Größe über paperless.max_upload_mb ergibt paperless_index_stub (Indexbeleg, Rolle index_stub); sonst Upload mit Titel aus dem Dateinamen, Dokumentdatum, Tag und Feldern, danach paperless_await_task mit fünf Sekunden Verzögerung. Ein Upload mit unklarem Ausgang (Verbindungsabbruch) wird nie blind wiederholt; der nächste Versuch beginnt mit der UUID-Suche. |
| 3 | paperless_await_task | verfolgt die Paperless-Aufgabe: PENDING, STARTED oder RETRY warten 30 Sekunden ohne Fehlversuch; SUCCESS legt die Verknüpfung mit Prüfsumme aus den Metadaten an (Zustand synced) und reiht paperless_push_meta ein; weicht die Prüfsumme vom lokalen SHA-256 ab, entsteht der Konfliktfall checksum_mismatch; meldet Paperless eine Dublette mit Kennung, wird die vorhandene Kopie verknüpft; sonst blockiert die Operation mit dem Ergebnistext |
| 4 | Ablage in Drive (Job file_to_drive, on_document_filed) | drive_set_props (Kennzeichen an der Drive-Datei und Drive-Verknüpfung, Rolle original), wenn die Anbindung aktiv oder das Drive-Änderungsprotokoll eingeschaltet ist; paperless_push_meta, wenn eine Paperless-Verknüpfung besteht und Schreiben erlaubt ist |
| 5 | paperless_push_meta | Sollwerte der vier Felder (UUID, Objektbezug, Zuordnungsstatus, Drive-Link) gegen synced_fields.app_fields; unverändert ergibt übersprungen ohne Schreibzugriff; sonst Felder setzen und Tag ergänzen, Audit sync.paperless_meta mit Vorher und Nachher |
| 6 | Objektübernahme (on_document_transferred) | Verknüpfungen und Versionen wandern auf die Nachfolgezeile; paperless_push_meta mit dem neuen Objektbezug |

Operationsliste (apps.sync.operations): enqueue legt idempotent an (gleicher Schlüssel, keine zweite Zeile; abgeschlossene Operationen werden nicht wiederholt, fehlgeschlagene, verworfene und blockierte beim nächsten Einreihen neu gestartet). Handler melden ihren Ausgang über Ausnahmen: Skip (nichts zu tun, Zustand skipped), Defer (Voraussetzung fehlt, zurück auf pending ohne Fehlversuch, Wartezeit je Grund 30 bis 900 Sekunden), Retry (vorübergehender Fehler, zählt als Versuch, Wartezeit 30 Sekunden je Versuch verdoppelt bis 3.600 Sekunden, bei Ratenbegrenzung der Wert aus Retry-After), Block (dauerhaft nicht lösbar ohne Eingriff, Zustand blocked). Nach sync.operation_max_attempts Versuchen endet eine Operation als failed. Der Beat-Eintrag sync-dispatch gibt jede Minute hängende Operationen frei (Heartbeat älter als 30 Minuten) und reiht fällige nach Priorität ein, die keine Nachricht unterwegs haben: send merkt dispatched_at, eine Operation gilt 60 Minuten als unterwegs, höchstens 200 sind gleichzeitig unterwegs; Retry, Defer und Freigabe setzen dispatched_at zurück (14.09.2026, zuvor wuchs die Warteschlange durch Mehrfachversand bis Redis voll war). Alle Operationen laufen in der Queue io (Dienst worker-io, Queues control, io, ai und lists; control traegt Sweep, Laufstart, Operationsversand, Token-Pflege und Statuspruefung, damit Steuerung nicht hinter einem Rueckstau in io wartet); Fehlerliste und letzte Operationen auf /verwaltung/sync/, Synchronisationsfehler auf /eingang/.

### 14.6 Ablauf Eingang: Paperless nach Anwendung

| Schritt | Inhalt |
|---|---|
| Regelmäßiger Abgleich | Beat-Eintrag sync-paperless-poll alle fünf Minuten; der Task prüft paperless.poll_interval_minutes und den Hauptschalter. Gelesen werden Dokumente mit modified nach dem Cursor modified_cursor (Sortierung modified, höchstens 2.000 je Lauf); je Dokument eine Operation paperless_pull mit Schlüssel aus Paperless-ID und modified. Der Cursor wird erst nach vollständiger Listung vorgeschoben; ein Fehler bleibt in last_poll sichtbar, nichts geht verloren. |
| Webhook | POST /webhooks/paperless/ ohne Sitzung; Geheimnis in der Kopfzeile X-MHV-Webhook-Token oder als Bearer-Token, Vergleich in konstanter Zeit; 401 mit Audit auth.denied bei fehlendem oder falschem Geheimnis, 404 bei paperless.webhook_enabled false, 400 ohne doc_id, 202 mit Operationsnummer. Je Dokument, Ereignis und Minute höchstens eine Operation (Priorität 50, vor dem regelmäßigen Abgleich). Der Body ist nur Auslöser; die Verarbeitung liest das Dokument selbst über die API. Der Abgleich holt verpasste Webhook-Ereignisse nach. |
| paperless_pull, bekanntes Dokument | Verknüpfung im Zustand tombstone wird übersprungen. 404 setzt missing und den Konfliktfall deleted_remote; deleted_at setzt trashed und trashed_remote. Sonst Vergleich mit dem zuletzt abgeglichenen Stand: abweichende Prüfsumme ergibt content_changed_remote und changed_remote; ein anderes Objekt im Feld Objekt ergibt object_changed_remote (Dokument in einem Objekt) oder einen Zuordnungsvorschlag from_paperless mit Bewertung 0,90 (Dokument im Eingang). Titel, Tags und modified werden in synced_fields fortgeschrieben. |
| paperless_pull, neues Dokument | Trifft das Feld Dokument-UUID ein lokales Dokument, wird nur verknüpft; trifft die Prüfsumme (original_checksum) ein lokales Dokument, ebenfalls (bei mehreren Treffern mit Vermerk). Ist das lokale Dokument bereits mit einer anderen Paperless-Kennung verknüpft, bleibt diese Verknüpfung bestehen und die zweite Kopie wird als Konfliktfall duplicate_remote gemeldet (nichts wird geladen, nichts umgehängt). Sonst, sofern paperless.import_new_documents an ist: Zielobjekt aus dem Feld Objekt (führende Ziffernfolge gegen object_number_numeric, nie das Eingangsobjekt) oder das Eingangsobjekt; Download des Originals in das Upload-Verzeichnis des Zielobjekts (Grenze documents.max_download_bytes, darüber blockiert); documents-Zeile mit source paperless und Status registered, Dokumentdatum aus created; Verknüpfung linked; Verarbeitungslauf des Zielobjekts (ensure_run). Exportfassungen und Indexbelege der Anwendung tragen UUID-Feld und Rolle und werden nie als eigenes Quelldokument übernommen. Dokumente mit Quelle Paperless werden nicht wieder nach Paperless hochgeladen; sie erhalten nach Ablage nur die Felder. |
| Pipeline im Eingangsobjekt | Hash, Seitenanalyse, OCR, Entitäten, Klassifikation wie üblich. Der Job decide legt für das Eingangsobjekt keine Fälle, Akten und Ablagen an, sondern hält nur Kategorie und Art fest und reiht den Job assign_object ein (Idempotenzschlüssel aus Objekt und SHA-256). |
| Objektzuordnung (apps.sync.flows.assign.run_for_document) | Datei ohne Drive-Kopie wird in den Eingangsordner gespiegelt (Wiederverwendung über appProperties.sha256, Fehler stoppt die Zuordnung nicht); Kandidaten mit Belegen nach 14.10; Entscheidung auto ergibt transfer_document in das Zielobjekt, Übernahme wartender Operationen auf die Nachfolgezeile, einen bereits erledigten Fall object_assignment mit Unterfall auto (Nachvollziehbarkeit) und Audit inbox.assign_auto; sonst Fall object_assignment mit Unterfall proposal (Kandidaten vorhanden) oder no_candidate, Kandidatenliste (höchstens fünf) mit object_id, object_number, score, evidence, contradictions, rule_hits, hints und proposed_action assign_object. |
| Übernahme in das Zielobjekt (transfer_document) | neue documents-Zeile im Zielobjekt (source moved_in) mit Seitentexten aus der Quelle, Altzeile moved_out mit Verweis und neuer UUID, Audit review.transfer_object beidseitig; im Zielobjekt Entitätenerkennung, Klassifikation, Entscheidung und Ablage (file_to_drive verschiebt die Drive-Datei aus dem Eingangsordner in die Zielstruktur, nie kopieren); danach die Hooks aus 14.5 (Kennzeichen, Felder). |

### 14.7 Drive-Änderungsprotokoll

apps.sync.drive_changes liest die Changes-API mit dauerhaftem Cursor sync_cursors (drive, page_token, meta drive_id). Ein Cursor ohne Vorgeschichte wird zuerst als Ausgangspunkt gesetzt (kein Rückblick, Audit sync.drive_changes mit baseline_cursor). Beat-Eintrag sync-drive-changes alle fünf Minuten, Task prüft sync.drive_changes_enabled und sync.drive_changes_interval_minutes, höchstens 20 Seiten je Lauf. Ein Resolver ordnet den Elternordner einer Änderung einem Objekt zu (Eingangsordner, Objektwurzeln, registrierte drive_nodes, sonst Elternkette bis Tiefe 8). Behandlung je Änderung: entfernt oder ohne Zugriff bei bekanntem Dokument setzt die Verknüpfung auf missing und den Konfliktfall drive_removed; Papierkorb setzt trashed und drive_trashed (nicht für duplicate oder moved_out); für ein als gelöscht markiertes Dokument (deleted_at gesetzt, etwa durch restformate_bereinigen) entsteht kein Konflikt: Papierkorb ist dort die gewollte Wirkung, die endgültige Entfernung nach Ablauf des Papierkorbs setzt die Verknüpfung auf tombstone, und eine Wiederherstellung der Datei aus dem Papierkorb (Verknüpfung trashed, Datei wieder aktiv) belebt das Dokument wieder (Löschkennzeichen zurück, Status registriert, Verknüpfung synced, Job discover, Audit sync.drive_restored; Ergänzung vom 26.09.2026); Ordner werden nur gezählt; bekannte Datei mit geändertem MD5 ergibt drive_content_changed und changed_remote; unveränderte bekannte Datei aktualisiert Elternordner und Namen in der Verknüpfung; unbekannte Datei in einem Objektordner oder im Eingangsordner wird als documents-Zeile mit source drive_existing registriert und erhält den Job discover; Dateien außerhalb des Umfangs werden gezählt. Ein ungültiger Cursor wird geleert, protokolliert und beim nächsten Lauf durch einen neuen Ausgangspunkt ersetzt; Änderungen dazwischen liefert nur ein Bestandslauf nach. Verschobene Ordner werden beim nächsten Bestandslauf nachgezogen. Nichts wird lokal oder in Drive gelöscht.

### 14.8 Bestandslauf

apps.sync.inventory führt ein wiederaufnehmbares Inventar je System (Art paperless oder drive), als Trockenlauf (Standard) oder echter Lauf, optional begrenzt auf Objektnummern. Je Art läuft höchstens ein Lauf gleichzeitig. Vor dem Drive-Lauf wird der Ausgangspunkt der Changes-API gesichert, vor dem Paperless-Lauf der aktuelle modified_cursor mit Startzeit (cursor_before), damit Änderungen während des Laufs vom anschließenden Abgleich erfasst werden. Der Lauf arbeitet in Paketen von sync.inventory_page_size Dateien je Schritt (Paperless höchstens 200 je Seite), verkettet über den Task sync.inventory_step, mit Seitenstand in page_state; pause, resume (auch nach failed), abort; ein Fehler im Schritt setzt failed mit Fehlertext statt einer Endlosschleife.

Paperless: bereits verknüpfte Dokumente ergeben link_existing; Papierkorb out_of_scope; Metadatenfehler error; Prüfsumme trifft genau ein lokales Dokument ergibt link_existing, mehrere duplicate; sonst import_new. Im echten Lauf entsteht je nicht verknüpftem Eintrag eine Operation paperless_pull mit Priorität 120 (nach dem laufenden Betrieb), der Eintrag steht auf in_progress. Drive: Wurzeln sind die Objektwurzeln (mit Umfang gefiltert) und ohne Umfang zusätzlich der Eingangsordner; Verknüpfungsdateien out_of_scope; registrierte Dateien link_existing (mit Paperless-Verknüpfung) oder import_new (ohne; im echten Lauf paperless_push, sofern das Objekt freigegeben ist und der Hash vorliegt); Google-Dokumente export_snapshot; unbekannte Dateien import_new und im echten Lauf Registrierung mit Job discover. Der Lauf gilt nur als vollständig (counters.complete), wenn er done erreicht und kein Eintrag pending oder error ist; offene Fehler werden nicht weggezählt. Die Detailseite überträgt den Ausgang der Operationen in das Manifest (refresh_dispositions). Trockenlauf schreibt nur das Manifest; vor dem Start wird lediglich der Cursorstand gesichert (cursor_before, beim Drive-Lauf bei Bedarf ein erster Ausgangspunkt der Changes-API).

Umfang je Objekt: Mit Objektnummern liest der Paperless-Lauf je Nummer nur die Dokumente, deren Feld MHV Objekt genau diesen Wert trägt (Filter custom_field_query, Seitenstand je Wert in page_state.scope_values und scope_index), statt des gesamten Bestands; der Drive-Lauf begrenzt auf die Objektwurzeln. Altbestand je Objekt (apps.sync.storage_paths): Der führende Zahlenblock eines Paperless-Speicherpfads gilt als Objektnummer und wird gegen die aktiven Objekte geprüft; je Pfad setzt die Anwendung das Feld MHV Objekt per bulk_edit in Paketen nur bei Dokumenten ohne Wert (nie überschreiben, abweichende Werte als Befund) und startet danach den echten Bestandslauf mit Umfang dieser Nummer. Stand je Pfad im Cursor field_fill:<Pfad-ID>, Speicherpfadliste im Cursor storage_paths (eine Stunde), Protokoll sync.paperless_field_fill, Aufgabe sync.storage_path_fill (Warteschlange io).

### 14.9 Konflikte

Konflikte sind Review-Fälle vom Typ sync_conflict, je Dokument, Unterfall und Anlass höchstens einer offen (batch_key), Priorität 60. Es gibt keine Regel, nach der der neueste Zeitstempel gewinnt; jede Entscheidung ist ausdrücklich, protokolliert und wirkt nur auf die Verknüpfung oder auf eine neue Operation.

| Unterfall | Erkennung | Zulässige Entscheidungen |
|---|---|---|
| content_changed_remote | Prüfsumme in Paperless weicht vom bekannten Stand ab | keep_local |
| duplicate_remote | zweite Kopie in Paperless (UUID-Feld oder Prüfsumme) zu einem Dokument, das bereits mit einer anderen Paperless-Kennung verknüpft ist | keep_local |
| object_changed_remote | Feld Objekt in Paperless nennt ein anderes Objekt (Dokument nicht im Eingang) | apply_remote_object, push_local_object |
| deleted_remote | Dokument in Paperless nicht mehr vorhanden | keep_local, tombstone, reupload |
| trashed_remote | Dokument in Paperless im Papierkorb | keep_local, tombstone |
| checksum_mismatch | Prüfsumme nach dem Upload weicht vom lokalen SHA-256 ab | keep_local, reupload |
| drive_trashed | Drive-Datei im Papierkorb | keep_local, tombstone |
| drive_removed | Drive-Datei entfernt oder kein Zugriff | keep_local, tombstone |
| drive_content_changed | MD5 der Drive-Datei weicht vom registrierten Stand ab | keep_local |

Entscheidungen (apps.sync.flows.conflicts): keep_local behält den lokalen Stand und vermerkt die Gegenseite als bekannt (neue Prüfsumme als akzeptiert); apply_remote_object übernimmt die Zuordnung aus Paperless über dieselbe Objektübernahme wie im Dokumenteneingang; push_local_object schreibt die eigene Zuordnung zurück (nur wenn Schreiben erlaubt); tombstone bestätigt die Löschung und setzt die dauerhafte Löschmarkierung, sodass dieselbe externe Kennung nie erneut übernommen wird; reupload verwirft die Verknüpfung und überträgt erneut. Keine Löschspiegelung: weder das lokale Dokument noch die Drive-Datei noch das Paperless-Dokument werden von der Anwendung gelöscht; der Paperless-Client besitzt keine Löschfunktion.

### 14.10 Objektzuordnung und Lernen

Mehrstufige Zuordnung (apps.sync.assignment, Module ohne Datenbankzugriff außer learning):

1. Normalisierung (normalize.py): Kleinschreibung, Umlaute und ß, Bindestrichvarianten, Straße, Str. und Strasse auf str, Hausnummern mit Zusatz, Bereichen (12-14) und zweitem Eingang (12/14), OCR-Verwechslungen (0/O, 1/l/I, 5/S, 8/B) nur im Ziffernkontext; 12, 12a und 12b bleiben verschieden. Jede Fundstelle einer bekannten Anschrift trägt eine Rolle aus dem Kontextfenster (object für Leistungsort oder Objektadresse, billing für Rechnungsempfänger, supplier für Absender und Impressumsangaben, owner für Eigentümer- oder Mieteranschrift, neutral ohne Rollenwort, own für die eigene Anschrift der Hausverwaltung) und die Angabe, ob Postleitzahl oder Ort im Umfeld bestätigt sind; gleiche Straße und Nummer in verschiedenen Orten bleibt ohne Bestätigung mehrdeutig.
2. Signale und Gewichte (candidates.py, WEIGHTS): Drive-Objektordner 0,88; Objektnummer mit Kontextwort im Text 0,85; Leistungsadresse 0,88; neutrale Adresse 0,80; Rechnungsadresse 0,30; Eigentümeranschrift 0,15; Lieferantenanschrift minus 0,20; Adresse oder Objektnummer im Dateinamen oder Paperless-Titel 0,60 und 0,50; Bestätigung durch Postleitzahl oder Ort je 0,15; Wiederholungstreffer 0,05 (höchstens zwei); bestätigte Regel 0,88; Verwaltungsart und Namen 0,05 und 0,10. Der Score ist die Noisy-Or-Verknüpfung der positiven Gewichte abzüglich der Gegenbelege, begrenzt auf 0 bis 1. Die eigene Anschrift ist nie Kandidat. Jeder Beleg trägt Art, Fundstelle, Seite, Gewicht und Rolle und steht im Fall.
3. Widersprüche (contradictions) und Hinweise: mehrere klare Leistungsadressen verschiedener Anschrift; Ordner- oder Objektnummernbezug gegen die Leistungsadresse eines anderen Objekts; gleiche Anschrift mehrerer Objekte (WEG und Sondereigentumsverwaltung): ist einer der Kandidaten über Ordner oder Objektnummer verankert, werden die Adressgewichte der übrigen halbiert und ein Hinweis erzeugt; sonst entsteht nur ein Hinweis auf die Verwaltungsart.
4. Entscheidung (decide.py): kein Kandidat mit positivem Score oder ein bester Kandidat unter 0,30 (review_min) ergibt none; Score unter sync.assignment_auto_min (0,85) ergibt review; Abstand zum zweiten Kandidaten unter sync.assignment_gap_min (0,25) ergibt review; ein Widerspruch am besten Kandidaten blockiert die Automatik; sonst auto. Die Gründe stehen als Text im Fall und im Audit.

Lernen (learning.py): Jede menschliche Entscheidung im Dokumenteneingang wird als Beispiel gespeichert (confirm, wenn das bestätigte Objekt dem Vorschlag entspricht, correct bei einem anderen Objekt, reject beim Verwerfen), mit Merkmalen (Lieferant aus dem Eintrag correspondent in synced_fields der Paperless-Verknüpfung, den der Eingangsablauf aus dem Korrespondenten des Paperless-Dokuments füllt; ohne Lieferant entsteht keine Merkmalskombination und damit keine Regel; Nummernpaare mit Kontextwort der Arten contract, customer, meter, insurance, property, normalisierte Adressen mit Rolle, Verwaltungsartwörter), Belegen, Person, Zeitpunkt und Fall. Automatische Zuordnungen sind keine Lernbeispiele. Der nächtliche Task sync.derive_rules (01:15 Uhr in der Zeitzone der Anwendung, Vorgabe Europe/Berlin) leitet Regeln ab: eine Merkmalskombination aus Lieferant, Nummernart und Nummer ergibt eine Regel feature_combo für genau ein Zielobjekt, wenn mindestens sync.rule_min_confirmations (2, technisch nie unter 2) bestätigte Beispiele aus verschiedenen Dokumenten vorliegen und kein Gegenbeispiel (Ablehnung des Ziels, Korrektur weg vom Ziel) existiert. Regeln sind versioniert (code, version); eine geänderte Regel ersetzt die aktive Vorgängerversion; Regeln sind abschaltbar (deactivate_rule) und sortieren bestätigte Altdokumente nie um. Beispiele der Quelle import_unverified zählen nicht, Beispiele mit features.holdout dienen nur der Messung (evaluate: Trefferquote der Automatik, Korrekturrate, Prüfaufwand).

### 14.11 Sicherheit

- Geheimnisse: API-Token und Webhook-Geheimnis liegen als Docker Secrets (PAPERLESS_TOKEN_FILE, PAPERLESS_WEBHOOK_TOKEN_FILE, gelesen in settings.OBJEKTAKTE), nie in app_settings, nie im Repository. Der Client protokolliert keine Header; für Diagnoseausgaben maskiert mask_headers den Authorization-Header. beat erhält die Paperless-Secrets nicht.
- Webhook: Paperless signiert Webhooks nicht; deshalb gemeinsames Geheimnis in der Kopfzeile X-MHV-Webhook-Token (oder Bearer), Vergleich in konstanter Zeit, 401 mit Audit auth.denied. Der Body wird nie als Wahrheit übernommen; erfundene Felder bleiben ohne Wirkung, weil die Verarbeitung das Dokument über die API liest und jede Operation idempotent ist.
- Rechte (9.1 ergänzt): sync.manage (Admin) für /verwaltung/sync/ mit Verbindungstest, Einrichtung der Kennzeichnung, Eingang einrichten (zusätzlich Step-up), Abgleich anstoßen, Bestandsläufe, Operationen erneut einreihen oder verwerfen; inbox.work (Admin und Sachbearbeiter) für den Dokumenteneingang /eingang/ und die Objektsuche; review.decide für die Entscheidungen in den Fällen; settings.write für die Schlüssel paperless.* und sync.*.
- Netz: Paperless wird nur aus Operationen und Tasks der Queue io (Dienst worker-io) und aus dem Verbindungstest in web aufgerufen. Das Netz egress haben in docker-compose.yml die Dienste web, worker (Queue ocr), worker-io und beat; die Paperless-Secrets erhalten web und die Worker-Dienste, beat nicht.
- Dokumente sind Daten, keine Anweisungen: Texte, Titel und Korrespondenten aus Paperless und Drive gehen ausschließlich als Eingabe in Mustervergleich und Bewertung ein; sie steuern keine Konfiguration, keine Rechte und keine Ausführung. Die Zuordnung arbeitet auf dem maskierten Seitentext; in Paperless-Felder, Indexbelege und Protokolle gelangen keine Personendaten.
- Audit: sync.paperless_check, sync.paperless_setup, sync.inbox_object_created, sync.inbox_folder_set, sync.paperless_push, sync.paperless_pull, sync.paperless_meta, sync.paperless_index_stub, sync.drive_snapshot, sync.drive_props_set, sync.drive_changes, sync.webhook, sync.conflict_resolved, sync.tombstone, sync.inventory_start, sync.inventory_pause, sync.inventory_resume, sync.inventory_done, sync.operation_retry, sync.operation_cancelled, inbox.assign, inbox.assign_auto, inbox.reject; Objektübernahmen zusätzlich review.transfer_object beidseitig.

### 14.12 Feature-Schalter und Modi

| Schalter | Vorgabe | Wirkung |
|---|---|---|
| paperless.enabled | aus | Hauptschalter. Aus stoppt jeden Abgleich und jeden Schreibzugriff nach Paperless (Schreiboperationen enden blockiert, die Pipeline reiht keine Übertragungen ein, Poll wird übersprungen, Webhook antwortet 202 mit ignored; bereits eingereihte Operationen jeder Art warten zurückgestellt und laufen nach dem Wiedereinschalten weiter); bereits gesicherte Daten, Verknüpfungen und Zustände bleiben. Verbindungstest und Trockenläufe brauchen nur Adresse und Token (configured), nicht den Hauptschalter (active). |
| paperless.mode | readonly | readonly: nur lesen und inventarisieren, Schreiben endet blockiert, Kennzeichnung wird nicht angelegt; pilot: Schreiben nur für Objekte in paperless.pilot_object_numbers (führende Nullen ohne Bedeutung) und für das Eingangsobjekt; full: alle Objekte. Geprüft in writes_allowed vor jedem Schreibzugriff (Upload, Felder, Indexbeleg, Exportfassung, Bestandslauf). |
| paperless.import_new_documents | an | Übernahme neuer Paperless-Dokumente; aus überspringt die Übernahme, Verknüpfung bekannter Dokumente bleibt möglich |
| paperless.import_only_with_object | an | Übernahme nur mit Feld MHV Objekt (direkt in das Objekt); ohne Feld bleibt das Dokument in Paperless, im Bestandslauf Disposition out_of_scope. Schutz vor der Übernahme eines DMS-Bestands ohne Objektbezug (Pilotbefund 13.09.2026: 96.145 von 96.265 Dokumenten) |
| paperless.webhook_enabled | an | Webhook-Endpunkt; aus liefert 404 |
| sync.drive_changes_enabled | aus | Drive-Änderungsprotokoll lesen; der Cursor bleibt beim Ausschalten gespeichert |

Empfohlene Reihenfolge der Inbetriebnahme: Adresse setzen, Verbindungstest im Modus readonly, Trockenläufe beider Bestände, Modus pilot mit Pilotobjekten, Kennzeichnung einrichten, Eingang einrichten, zuletzt Hauptschalter an; Umfang erst nach Sichtung von Fehlerliste und Dokumenteneingang erweitern (docs/betrieb/paperless-sync.md, Abschnitte 3 und 6).

### 14.13 Abgrenzung

- Import-Parser (CR 15, Abschnitt 8.5): Stammdaten (Eigentümer, Einheiten, Mieter, Zuordnungen) entstehen weiterhin nur über Import und Bestätigung im Review Center. Die Synchronisation legt ausschließlich Dokumente, Verknüpfungen, Operationen und Fälle an. Ein aus Paperless übernommenes Dokument, das eine Eigentümer- oder Mieterliste ist, durchläuft dieselbe Erkennung wie jeder Upload und erzeugt den Fall import_candidate; der Import bleibt eine Entscheidung.
- Altbestandslauf (Entscheidungen vom 11.09.2026, Bestand aus Drive übernehmen, takeover_sources): Er strukturiert alte Ordner objektweise um (Verschieben in die neue Struktur, Aufräumen nach Vorschau). Der Bestandslauf der Synchronisation inventarisiert dagegen den Paperless-Bestand und die registrierten Drive-Strukturen (Objektwurzeln und Eingangsordner), verknüpft oder registriert Dateien und stellt die Zweitkopie in Paperless her; er ordnet keine Altordner um, räumt nichts auf und legt keine Objekte an. Für die Ordnerstruktur bleibt der Ordnerabgleich (Abschnitt 7.3) zuständig.
- Drive-Ablage (Abschnitt 7.5): Die Synchronisation verschiebt Dateien nur über die vorhandene Ablage (file_to_drive) und setzt sonst nur appProperties; Schreibidempotenz und Sperren der Ablage gelten unverändert.
